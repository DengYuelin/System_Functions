...
bool Direct3D_Init(HWND window, int width, int height, bool fullscreen)
{
	...
	d3dpp.FullScreen_RefreshRateInHz = NULL;			//edite this when using fullscreen
...
void FontPrint(LPD3DXFONT font, int x, int y, int w, int h, string text, D3DCOLOR color)
{
	...
	RECT rect_1 = { x, y, 0, 0 };
	font->DrawText(NULL, text.c_str(), text.length(), &rect_1, DT_CALCRECT, color);

	//print the text
	font->DrawText(spriteobj, text.c_str(), text.length(), &rect_1, DT_LEFT, color);
}
